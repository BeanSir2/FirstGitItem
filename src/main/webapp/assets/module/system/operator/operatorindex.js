$(function($) {
	...
	$("#importexecl").click(function() {
		var node = getSelcetedNode();
		if(node==null || node.attr("id")==null) {
			Common.messageBox("提示", "请先选择左侧公司！", false, 2000);
		}
		var companyId = node.attr("id").substring(node.attr("id").indexOf("company_")+8);
		console.log(companyId);
		dialog = $("#importOperator").removeClass('hide').dialog({
			modal : true,
			...
			buttons : [ {
				...
				click : function() {
					if(Common.isEmpty(companyId)){
						Common.messageBox("提示", "请选择左侧的公司节点！", false);
						return false;
					}
					...
					$.ajax({
						...
						dataType : "json",
						success : function(json) {
							$("#importOperator").dialog("close");
							Common.hideMask();
							if(json.isSuccess){
								Common.messageBox("提示", json.msg, true,'',true);
								Common.jqGridReload("grid-table");
								$(this).dialog("close");
							}else{
								Common.messageBox("提示", json.msg, false,'',true);
							}
						},
						error : function() {
							Common.hideMask();
